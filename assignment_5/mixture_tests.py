...
class GMMTests(unittest.TestCase):
    ...
    def test_bayes(self, BIC_likelihood_model_test, improved_initialization):
        """
        Test for your
        implementation of
        BIC on fixed GMM values.
        ...
        comp_means = []
        for k in range(5,10):
            MU, SIGMA, PI = improved_initialization(image_matrix,k)
            comp_means.append(MU)
        
        n_comp_min_bic, n_comp_max_likelihood = BIC_likelihood_model_test(image_matrix, comp_means)

        print("n_comp_min_bic ", n_comp_min_bic)
        print("n_comp_max_likelihood ", n_comp_max_likelihood)
